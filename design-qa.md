...
The empty-secret state preserves the existing local flow. A live `http://localhost:11434` test completed successfully without an Authorization header, returned five models, and retained the selected model. At compact sidebar heights, the connection panel scrolls independently; the API-key selector and Test endpoint action remain reachable without moving the composer.
